...
<div class="flex items-start gap-4"><div class="flex size-11 items-center justify-center rounded-xl bg-slate-900 text-white"><Tags class="size-5" /></div><div><h1 class="text-2xl font-semibold">Edit Device Type</h1><p class="mt-1 text-sm text-slate-500">Update the device type configuration.</p></div></div>
...
<div class="flex justify-end gap-3"><Link :href="laraiotUrl('settings/device-types')" class="rounded-lg border px-4 py-2.5 text-sm">Cancel</Link><button :disabled="form.processing || !form.isDirty" class="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm text-white"><Save class="size-4" />Save Changes</button></div>
</form>
</div></LaraIoTLayout>
</template>
