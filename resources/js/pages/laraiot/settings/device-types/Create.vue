...
<div class="flex items-start gap-4"><div class="flex size-11 items-center justify-center rounded-xl bg-slate-900 text-white"><Tags class="size-5" /></div><div><h1 class="text-2xl font-semibold">Add Device Type</h1><p class="mt-1 text-sm text-slate-500">Create a logical device type.</p></div></div>
<form class="space-y-6" @submit.prevent="submit">
<section class="space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
<div><label class="text-sm font-medium">Identifier</label><input v-model="form.identifier" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2.5 font-mono text-sm" /><p v-if="form.errors.identifier" class="mt-1 text-sm text-red-600">{{ form.errors.identifier }}</p></div>
<div><label class="text-sm font-medium">Name</label><input v-model="form.name" class="mt-2 block w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm" /></div>
...
<div class="flex justify-end gap-3"><Link :href="laraiotUrl('settings/device-types')" class="rounded-lg border px-4 py-2.5 text-sm">Cancel</Link><button :disabled="form.processing" class="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm text-white"><Save class="size-4" />Save Device Type</button></div>
</form>
</div></LaraIoTLayout>
</template>
