...
<div class="flex items-start gap-4"><div class="flex size-11 items-center justify-center rounded-xl bg-slate-900 text-white"><Radio class="size-5" /></div><div><h1 class="text-2xl font-semibold">Add MQTT Topic</h1><p class="mt-1 text-sm text-slate-500">Configure a state or command topic.</p></div></div>
<form class="space-y-6" @submit.prevent="submit"><MqttTopicForm :form="form" /><div class="flex justify-end gap-3"><Link :href="laraiotUrl(`devices/logical/${logicalDevice.id}`)" class="rounded-lg border px-4 py-2.5 text-sm">Cancel</Link><button :disabled="form.processing" class="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm text-white"><Save class="size-4" />Save MQTT Topic</button></div></form>
</div></LaraIoTLayout>
</template>
